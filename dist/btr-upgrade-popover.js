...
!function ($) {
    ...
    $.fn.btrUpgradePopover.defaults = $.extend({}, $.fn.popover.defaults, {
      ...
      , contentTypes: {
          duration: '<p>Never cut your show short again. Upgrade to premium to fully express yourself. Get up to 3 hours of airtime every day.</p><p align="center"><span class="icon-premium-primetime"></span></p><p><a href="{{link}}" class="btn btn-warning btn-large input-block-level" data-tracker-category="PremiumAd" data-tracker-action="Popover Click" data-tracker-label="Long Duration">{{linkLabel}}</a></p>',
          import: '<p>Importing enables you to edit &amp; replace your shows, copy information from your blog or publish pre-recorded episodes.</p><div class="row-fluid"><div class="span6"><span data-btr-icon="H" class="upload"></span><p>Upload Episodes</p></div><div class="span6"><span data-btr-icon="Q" class="rss"></span><p>Import by RSS</p></div></div><p><a href="{{link}}" class="btn btn-warning btn-large input-block-level" data-tracker-category="PremiumAd" data-tracker-action="Popover Click" data-tracker-label="Import">{{linkLabel}}</a></p>',
          primetime: '<p>Expose your show to a bigger audience with Prime Time Scheduling. Go live during our busiest time of the day when the most people are listening.</p><p align="center"><span class="icon-premium-primetime"></span></p><p><a href="{{link}}" class="btn btn-warning btn-large input-block-level" data-tracker-category="PremiumAd" data-tracker-action="Popover Click" data-tracker-label="Primetime">{{linkLabel}}</a></p>'
        }
    })
